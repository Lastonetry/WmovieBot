...
async def start(client:Client, message): 
            ...
            if refData and refData.split("-", 1)[0] == "biisal":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        buttons = [[
                            ...
                            [
                            InlineKeyboardButton('♻️ ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ ғʀᴇᴇ ♻️', callback_data=f'free_premium#{message.from_user.id}')
                            ],
                            [
    ...
    if data and data.startswith("allfiles"):
        ...
        for file in files:
            ...
            btn = [[
                InlineKeyboardButton("🚀 ғᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ/ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 💻", callback_data=f'stream#{file.file_id}')
            ]]
    ...
    btn = [[
        InlineKeyboardButton("🚀 ғᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ/ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 💻", callback_data=f'stream#{file_id}')
    ]]
    toDel=await client.send_cached_media(
        chat_id=message.from_user.id,
        file_id=file_id,
...
async def delete_all_index(bot, message):
    ...
    if int(files) == 0:
        return await message.reply_text('Database empty! No files to delete')
    btn = [[
...
async def settings(client, message):
    ...
    if not user_id:
        return await message.reply("<b>💔 ʏᴏᴜ ᴀʀᴇ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪɴ, ᴋɪɴᴅʟʏ ᴛᴜʀɴ ᴏғғ ᴀɴᴏɴʏᴍᴏᴜs ᴍᴏᴅᴇ ᴛʜᴇɴ ᴛʀʏ ᴀɢᴀɪɴ...</b>")
...
async def search_files(bot, message):
    ...
    await message.reply_document(
        ...
        caption=f"<b>♻️ ғʀᴏᴍ ʏᴏᴜʀ ꜱᴇᴀʀᴄʜ, ɪ ꜰᴏᴜɴᴅ - <code>{total}</code> ꜰɪʟᴇs</b>",
        parse_mode=enums.ParseMode.HTML
    )
...
async def delete_files(bot, message):
    ...
    if deleted_files_count > 0:
        await message.reply_text(f'<b>{deleted_files_count} files successfully deleted from the database 💥</b>')
    if not_found_files:
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/ThaFallenStar</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/ThaFallenStar').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
        await save_group_settings(grp_id, 'shortner', URL)
        await save_group_settings(grp_id, 'api', API)
        await m.reply_text(f"<b><u>✓ sᴜᴄᴄᴇssꜰᴜʟʟ! ʏᴏᴜʀ sʜᴏʀᴛɴᴇʀ ɪs ᴀᴅᴅᴇᴅ</u>\n\nᴅᴇᴍᴏ - {SHORT_LINK}\n\nsɪᴛᴇ - `{URL}`\n\nᴀᴘɪ - `{API}`</b>", quote=True)
        user_id = m.from_user.id
...
async def set_shortner_2(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴀᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/ThaFallenStar>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
