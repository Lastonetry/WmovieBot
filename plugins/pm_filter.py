...
async def pm_search(client, message):
    ...
    if ispm:
        ...
        text=f"<b>ʜᴇʏ {message.from_user.mention},\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇ ʜᴇʀᴇ\nɪ ᴏɴʟʏ ᴡᴏʀᴋ ɪɴ ɢʀᴏᴜᴘs, ᴄʀᴇᴀᴛᴇ ᴀ ɴᴇᴡ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀᴅᴅ ᴍᴇ ɪɴ ɪᴛ, ᴏʀ ᴊᴏɪɴ ᴍʏ ɢʀᴏᴜᴘ, ʙʏ ᴄʟɪᴄᴋɪɴɢ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ</b>",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "ᴊᴏɪɴ ᴍᴏᴠɪᴇs ɢʀᴏᴜᴘ", url=link
                    )
                ]
            ]
        ),
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        await query.answer("ʙʀᴜʜ, ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇs!", show_alert=True)

    elif query.data == "pages":
        await query.answer("ᴡʜʏ sᴏ ᴄᴜʀɪᴏᴜs ? 🌚")
        ...
        if str(userid) in user_id:
            await query.answer("sᴏʀʀʏ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ɪs ʀᴇᴊᴇᴄᴛᴇᴅ", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("ᴘʟᴇᴀꜱᴇ ᴍᴇɴᴛɪᴏɴ ʀᴇʟᴇᴀsᴇ ʏᴇᴀʀ ᴀɴᴅ ʟᴀɴɢᴜᴀɢᴇ, sᴏ ᴛʜᴀᴛ ᴡᴇ ᴄᴀɴ ғɪɴᴅ ᴍᴏʀᴇ ᴀᴘᴘʀᴏᴘʀɪᴀᴛᴇ ʀᴇsᴜʟᴛs ‼️", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                await delSticker(st)
                ai_sts = await msg.reply_text('<b>ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ sᴘᴇʟʟ ʙʏ ᴀɪ-ᴘᴏᴡᴇʀᴇᴅ sʏsᴛᴇᴍ....</b>')
                is_misspelled = await ai_spell_check(search)
                if is_misspelled:
                    await ai_sts.edit(f'<b>ᴀɪ sᴜɢɢᴇsᴛᴇᴅ <code>{is_misspelled}</code>\nsᴏ ᴀᴍ sᴇᴀʀᴄʜɪɴɢ ғᴏʀ <code>{is_misspelled}</code></b>')
    ...
    del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
    ...
    if imdb:
        ...
        cap = f"<b>📂 <u>ʜᴇʀᴇ ɪ ꜰᴏᴜɴᴅ ꜰᴏʀ ʏᴏᴜʀ sᴇᴀʀᴄʜ {search}</u></b>"
    del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    CAP[key] = cap
